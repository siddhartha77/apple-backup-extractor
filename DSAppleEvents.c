...
void FailErr(OSErr err) {
	...
	if (err != noErr)
		ErrorAlert(kErrStringID, kAEVTErr, ERROR(err));
}
...
pascal OSErr _HandleDocs (AppleEvent *theAppleEvent, AppleEvent *reply, Boolean opening)
{
	...
	if (itemsInList > 1)
	{
	    ...
	    CenterAlert(kAlertMultipleFiles);
    	ParamText
    	(
    	    myFSS.name,
    	    NULL,
	    ...
	    NoteAlert(kAlertMultipleFiles, NULL);	    
	}
	
	if (PreFlightDocs (opening, itemsInList, &userDataHandle))
	{	// let the app do any preflighting it might need
